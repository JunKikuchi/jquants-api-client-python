...
`ClientV2` は 429 (Too Many Requests) および 5xx エラー時に指数バックオフ付きで自動リトライします。
デフォルトでは最大10回、バックオフ係数1（1s, 2s, 4s, 8s, ... と待機時間が増加）で設定されています。

環境変数または設定ファイルでリトライ設定を変更できます。
...
クライアント側のレートリミッターおよびリトライ機能により多くの場合は自動的に処理されますが、
